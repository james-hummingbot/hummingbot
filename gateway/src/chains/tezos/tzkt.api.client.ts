...
export class TzktApiClient {
  ...
  async getAccountStatus(address: string): Promise<AccountStatusResponse> {
    return axios.get(`${this._tzktURL}/v1/accounts/${address}`);
  }
  ...
  async getTokens(
    ...
    return axios.get(
      `${this._tzktURL}/v1/tokens/balances?account=${walletAddress}&token.contract=${contractAddress}&token.tokenId=${tokenId}`
    );
  }

  async getTransaction(txHash: string): Promise<TransactionResponse> {
    return axios.get(`${this._tzktURL}/v1/operations/transactions/${txHash}`);
  }

  async getBlockHead(): Promise<BlockHeadResponse> {
    return axios.get(`${this._tzktURL}/v1/head`);
  }
}
